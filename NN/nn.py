...
def InitNN(num_inputs, num_hiddens, num_outputs):
    ...
    dW1 = np.zeros((num_inputs,num_hiddens[0]))
    dW2 = np.zeros((num_hiddens[0], num_hiddens[1]))
    dW3 = np.zeros((num_hiddens[1], num_outputs))
    db1 = np.zeros((num_hiddens[0]))
    db2 = np.zeros((num_hiddens[1]))
...
def AffineBackward(grad_y, x, w):
    ...
    #grad_x = np.dot(np.transpose(w),grad_y)
    grad_x = grad_y.dot(w.T)
    #grad_w = np.dot(grad_y,np.transpose(x))
