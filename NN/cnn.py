...
def InitCNN(num_channels, filter_size, num_filters_1, num_filters_2,
    ...
    dW1 = np.zeros((filter_size, filter_size, num_channels,  num_filters_1))
    dW2 = np.zeros((filter_size, filter_size, num_filters_1, num_filters_2))
    dW3 = np.zeros((num_filters_2 * 64, num_outputs))
    db1 = np.zeros((num_filters_1))
    db2 = np.zeros((num_filters_2))
    db3 = np.zeros((num_outputs))
...
def Conv2DBackward(grad_y, x, y, w):
    ...
    I = len(w)
    J = len(w[0])
    grad_w = Conv2D(np.transpose(x, [3,1,2,0]), np.transpose(grad_y, [1,2,0,3]), [I-1,J-1])
