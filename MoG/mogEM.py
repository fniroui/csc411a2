...
def q2():
    ...
    ShowMeans(mu, 1)
    ShowMeans(vary, 2)
    print 'probabilities of clusters:\n %s' % p
    #------------------- Answers ---------------------
...
def q4():
    ...
    for t in xrange(T):
        ...
        #-------------------- Add your code here ------------------------------
        pAnger, muAnger, varAnger, loglikeAnger_train = mogEM(x_train_anger, K, iters, randConst, minVary)
        pHappy, muHappy, varHappy, loglikeHappy_train = mogEM(x_train_happy, K, iters, randConst, minVary)
        ...
        #-------------------- Add your code here ------------------------------
        loglikeAnger_train = mogLogLikelihood(pAnger, muAnger, varAnger, x_train) + log_likelihood_class[0]
        loglikeHappy_train = mogLogLikelihood(pHappy, muHappy, varHappy, x_train) + log_likelihood_class[1]

        loglikeAnger_val = mogLogLikelihood(pAnger, muAnger, varAnger, x_valid) + log_likelihood_class[0]
        loglikeHappy_val = mogLogLikelihood(pHappy, muHappy, varHappy, x_valid) + log_likelihood_class[1]

        loglikeAnger_test = mogLogLikelihood(pAnger, muAnger, varAnger, x_test) + log_likelihood_class[0]
        loglikeHappy_test = mogLogLikelihood(pHappy, muHappy, varHappy, x_test) + log_likelihood_class[1]

        ptrain = (loglikeAnger_train < loglikeHappy_train)
        pval = (loglikeAnger_val < loglikeHappy_val)
        ptest = (loglikeAnger_test < loglikeHappy_test)

        errorTrain[t] = np.sum((ptrain != y_train).astype(float)) / len(y_test)
        errorValidation[t] = np.sum((pval != y_valid).astype(float)) / len(y_test)
        errorTest[t] = np.sum((ptest != y_test).astype(float)) / len(y_test)
